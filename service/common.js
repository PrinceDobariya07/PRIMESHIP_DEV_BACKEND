...
module.exports = {
  ...
  findService: (countryId, weight, length, width, height) => {
    return new Promise(async (res, rej) => {
      try {
        let totalGram = ((length * width * height) / 5000) * 1000;
        let parcelGram = Math.max(totalGram, weight)

        if (parcelGram % 100 !== 0) parcelGram = parcelGram - parcelGram % 100 + 100

        // console.log("totalGram: ", totalGram)
        // let z = totalGram / 100;
        // let floorVal = Math.floor(z);
        // if (Number.isInteger(z)) {
        //   floorVal = floorVal * 100;
        // } else {
        //   console.log("coming here..is not integer: ")
        //   console.log("before: ", floorVal)

        //   floorVal = floorVal * 100 + 100;
        //   console.log("after: ", floorVal)
        // }
          ...
          if (creditData.credit >= data.amount) {
            ...
            if (savedata) {
              let updatedData = await userModel.findByIdAndUpdate(
                { _id: userId },
                { credit: creditData.credit - savedata.grandTotal },
                { new: true }
              );
