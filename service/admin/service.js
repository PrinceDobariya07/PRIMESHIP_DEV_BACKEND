...
module.exports = {
    ...
    addPriceChart: (userId, data) => {
        return new Promise(async (res, rej) => {
            try {
                // data.adminId = userId
                let existData = await priceChartModel.find({ serviceId: data.serviceId, countryId: data.countryId });
                if (existData.length > 0) {
                    let findData = await priceChartModel.deleteMany({ serviceId: data.serviceId, countryId: data.countryId });
                }
                let i
