  ...
  get: (str, status, startDate, endDate, page, limit) => {
    return new Promise(async (res, rej) => {
      try {
        ...
        var result = orders.filter(function(o1){
          // filter out (!) items in result2
          return !admins.some(function(o2){
              return o1.adminId === o2._id;          // assumes unique id
          });
      })
     // console.log(result)
      ...
      try {
        ...
        if (getorderData) {
          if (
            ...
            if (data.orderTrackingStatus === "pending") {
              let orderData = await orderModel.findByIdAndUpdate(
                { _id: getorderData._id },
                {
  ...
  extraCharge: (userId, orderId, amount, message) => {
    return new Promise(async (res, rej) => {
      try {
        ...
        if (check.extraCharge === 0 || check.extraCharge > 0) {
          newAmount = amount + check.extraCharge;
        }
        let orderData = await orderModel.findByIdAndUpdate(
          orderId,
          { extraCharge: newAmount },
          { new: true }
        );
        let grandTotal = check.amount + newAmount;
        let orderHisData = await orderHistoryModel.findOneAndUpdate(
          { orderId: orderId },
          {
            extraCharge: newAmount, grandTotal: grandTotal,
